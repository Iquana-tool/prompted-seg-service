...
class Prompted2DSegmentationRequest(BaseModel):
    ...
    @field_validator('point_prompts', 'box_prompt', 'circle_prompt', 'polygon_prompt', mode='before')
    def validate_at_least_one_prompt(cls, value, values, **kwargs):
        if not any([values.get('point_prompts'), values.get('box_prompt'),
                    values.get('circle_prompt'), values.get('polygon_prompt')]):
            raise ValueError("At least one prompt (point, box, circle, or polygon) must be provided.")
        return value
